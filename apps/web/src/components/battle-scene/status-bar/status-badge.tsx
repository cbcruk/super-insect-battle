...
export function StatusBadge({
  ...
  return (
    <div className="flex gap-1">
      {condition && (
        <Badge variant="outline" className={`rounded px-1.5 py-0.5 text-[10px] font-bold border-transparent ${conditionStyles[condition].color}`}>
          {conditionStyles[condition].label}
        </Badge>
      )}
      {battleMode && (
        <Badge variant="outline" className={`rounded px-1.5 py-0.5 text-[10px] font-bold border-transparent ${modeStyles[battleMode].color}`}>
